...
  pop();
  ...
  if (isQuizActive && currentQuestionIndex >= QUIZ_QUESTIONS.length) {
      ...
      if (!quizEndText) { // 只設定一次結束文字
        ...
        const finalMsgs = [
            `測驗結束! 喵～你總共答對了 ${finalScore} 題，總共 ${totalQuestions} 題。表現得真棒喵！`,
            `恭喜完成喵！你獲得了 ${finalScore} 分，感謝你的參與喵！`,
        ];
        quizEndText = random(finalMsgs);
  ...
  if (isPlayerNearOrangeCat && (gameMode === 'explore' || gameMode === 'quizEnd' || isQuizActive)) {
    if (healState === 'idle') healState = 'asking'; // 如果靠近且閒置，則開始詢問
  ...
  // 優先權 3: 小紅的對話框 (新增)
  if (isPlayerNearXiaoHong && (gameMode === 'explore' || gameMode === 'quizEnd')) {
    if (xiaoHongState === 'idle') xiaoHongState = 'request_help'; // 【修改】初始狀態改為請求協助
    // 【修改】如果狀態不是 dismissed (被玩家關閉)，才顯示對話
    if (xiaoHongState !== 'dismissed') drawXiaoHongDialogue();
  } else {
    xiaoHongState = 'idle';
    currentXiaoHongQuestion = null; // 【新增】離開時重置題目，確保對話完全終止
  }
  ...
  if (isPlayerNear && !isQuizActive && !quizEndText && !isPlayerNearXiaoHong && !isPlayerNearJiuJiu) { // 避免重疊
    // 【修改】如果沒有被手動關閉，才顯示提示
    if (!isPurpleCatDismissed) drawStartPrompt();
...
function mousePressed() {
  ...
  if (isPlayerNearXiaoHong && (gameMode === 'explore' || gameMode === 'quizEnd')) {
    if (xiaoHongState === 'request_help') { // 【新增】處理請求協助的按鈕
      ...
      const btnClose = closeButton;
      if (mouseX > btnClose.x && mouseX < btnClose.x + btnClose.w && mouseY > btnClose.y && mouseY < btnClose.y + btnClose.h) {
        xiaoHongState = 'post_quiz'; // 進入詢問下一步的狀態
        return;
      }
      ...
      if (mouseX > btn1.x && mouseX < btn1.x + btn1.w && mouseY > btn1.y && mouseY < btn1.y + btn1.h) {
        ...
        currentXiaoHongQuestion = null; // Reset question
        return;
      }
      // 選項 2: 去找別人
      const btn2 = xiaoHongOpt2Button;
...
function drawXiaoHongDialogue() {
  ...
  if (xiaoHongState === 'request_help') { // 【新增】請求協助對話
    ...
    if (!currentXiaoHongQuestion) {
      currentXiaoHongQuestion = random(XIAO_HONG_QUESTIONS);
    }
    const questionText = "小紅問：\n" + currentXiaoHongQuestion.question;
